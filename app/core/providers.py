...
PROVIDERS: dict[str, dict[str, str]] = {
    ...
    "paystack": {"label": "Paystack", "color": "#00c3f7", "logo": "/static/img/providers/paystack.svg"},
    ...
    "sanity": {"label": "Sanity", "color": "#f03e2f", "logo": "/static/img/providers/sanity.svg"},
    "emailjs": {"label": "EmailJS", "color": "#f59e0b", "logo": "/static/img/providers/emailjs.svg"},
...
_VALUE_KIND_RULES: tuple[tuple[str, str, re.Pattern[str]], ...] = (
    ("paystack", "paystack", re.compile(r"^sk_(?:live|test)_[0-9a-f]{40}$", re.I)),
    ("stripe", "stripe_live", re.compile(r"^sk_live_[0-9a-zA-Z]{24,}$")),
    ("stripe", "stripe_test", re.compile(r"^sk_test_[0-9a-zA-Z]{24,}$")),
...
def classify_env_assignment(key: str, value: str) -> tuple[str, str, str] | None:
    ...
    Brevo / Paystack do not appear twice.
    """
    ...
    # Explicit env key wins (PAYSTACK_SECRET_KEY=sk_test_… must not become Stripe).
    for marker, provider, kind in _ENV_KEY_PROVIDERS:
        if marker in key_l:
            ...
            if provider == "stripe":
                ...
                if rhs.startswith("sk_test_"):
                    return "stripe", "stripe_test", rhs
                return "stripe", "stripe", rhs
            if provider == "paystack":
                return "paystack", "paystack", rhs
            return provider, kind or provider, rhs

    # Value-shape fallback — Paystack (40 hex) is listed before Stripe.
